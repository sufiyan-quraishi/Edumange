...
<head>
    ...
    <title>Student Directory - EduManage</title>
    ...
    <style>
        ...
        .custom-student-table tr:last-child td {
            border-bottom: none;
        }
        .custom-student-table tr:hover {
            ...
            <div class="header-bar" style="margin-bottom: 24px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px;">
                <div>
                    <h2 style="margin: 0; font-size: 22px; font-weight: 800; color: var(--text, #0f172a);">
                        <i class="fas fa-user-graduate" style="color: var(--primary, #4f6ef7); margin-right: 8px;"></i> Student Management
                    </h2>
                    <span style="color: var(--text-muted, #64748b); font-size: 13.5px;">Manage enrolled student records, account statuses, and system credentials.</span>
                </div>
                <a href="${pageContext.request.contextPath}/admin/dashboard" style="text-decoration: none;">
                    <button type="button" class="mini-btn" style="padding: 9px 16px; font-size: 13px; font-weight: 600; cursor: pointer;">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid var(--border, #e2e8f0); padding-bottom: 14px; margin-bottom: 10px;">
                    <div>
                        <h5 style="margin: 0; font-size: 16px; font-weight: 700; color: var(--text, #1e293b);">
                            <i class="fas fa-users" style="color: var(--primary, #4f6ef7); margin-right: 6px;"></i> Enrolled Students Directory
                        </h5>
                        <small style="color: var(--text-muted, #64748b); font-size: 12.5px;">All active and inactive student accounts</small>
                    </div>
                    <span class="badge badge-approved" style="font-size: 12px; font-weight: 700;">
                        <i class="fas fa-check-circle" style="margin-right: 4px;"></i> Total: ${fn:length(students)}
                    </span>
                </div>

                <div class="table-responsive">
                    <table class="custom-student-table">
                        <thead>
                            <tr>
                            ...
                            <c:choose>
                                <c:when test="${not empty students}">
                                    <c:forEach var="s" items="${students}">
                                        <tr>
                                            <td>
                                                <strong style="color: var(--text-muted, #64748b); font-size: 12.5px;">#${s.id}</strong>
                                            </td>
                                            <td>
                                                <strong style="color: var(--text, #0f172a); font-size: 14px;">${s.fullName}</strong>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                <span style="color: var(--text, #334155); font-size: 13px;">${s.email != null ? s.email : '-'}</span>
                                            </td>
                                            <td>
                                                <span style="color: var(--text, #334155); font-size: 13px;">${s.phone != null ? s.phone : '-'}</span>
                                            </td>
                                            ...
                                            <td style="text-align: center;">
                                                <div style="display: inline-flex; align-items: center; gap: 8px; justify-content: center;">
                                                    <a href="${pageContext.request.contextPath}/admin/students/edit/${s.id}" class="btn-action-edit">
                                                        <i class="fas fa-edit"></i> Edit
                                                    </a>

                                                    <form action="${pageContext.request.contextPath}/admin/students/delete/${s.id}" method="post" style="display: inline; margin: 0;" onsubmit="return confirm('Are you sure you want to delete student: ${s.fullName}?');">
                                ...
                                <c:otherwise>
                                    <tr>
                                        <td colspan="7" style="text-align: center; padding: 28px; color: var(--text-muted, #64748b);">
                                            <i class="fas fa-user-slash" style="margin-right: 6px; font-size: 18px;"></i> No students registered in the system.
                                        </td>
                                    </tr>
                                </c:otherwise>
                            </c:choose>
